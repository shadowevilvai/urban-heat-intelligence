...
def export_spatial_lst(city_key, scale=450):
    """
    Exports a coarse sample of the environmental composite as GeoJSON points.

    Includes LST, historical anomalies, and Sentinel-2 / ESA environmental indices,
    ensuring spatial alignment by sampling them simultaneously from a single Image.
    """
    composite, _ = get_lst_composite(city_key)
    aoi = get_aoi(city_key)

    # Sample the image at a coarser scale (e.g., 450m) to reduce output size.
    # dropNulls is true by default, so masked pixels (e.g., cloud-covered LST) are excluded.
    ...
    import datetime
    process_date_str = datetime.datetime.now().strftime("%Y-%m-%d")
    ...
    for feat in features_ee:
        ...
        if lst is not None:
            ...
            lst_anomaly = props.get('LST_Anomaly_Celsius')
            ndvi = props.get('NDVI_Mean')
            ndbi = props.get('NDBI_Mean')
            ndwi = props.get('NDWI_Mean')
            lc_code = props.get('Land_Cover_Class')

            lc_class_str = None
            if lc_code is not None and lc_code in ESA_WORLDCOVER_MAP:
                lc_class_str = ESA_WORLDCOVER_MAP[lc_code]
            ...
            geojson_features.append({
                ...
                "properties": {
                    ...
                    "city": city_key,
                    "lst_c": round(lst, 2),
                    "lst_anomaly_c": round(lst_anomaly, 2) if lst_anomaly is not None else None,
                    "source": "Landsat 8/9 C02 L2, Sentinel-2 SR, ESA WorldCover v200",
                    "satellite": "Multiple",
                    "date_period_start": DEFAULT_DATE_RANGE["start"],
                    "date_period_end": DEFAULT_DATE_RANGE["end"],
                    "processing_date": process_date_str,
                    "resolution_m_source": 30, # Representative base resolution
                    "resolution_m_sample": scale,
                    "coordinate_reference_system": "EPSG:4326",
                    "data_quality": "validated",
                    "valid_pixel_percent": round(props.get('Valid_Pixel_Percent', 0), 2) if props.get('Valid_Pixel_Percent') is not None else None,
                    "ndvi_mean": round(ndvi, 3) if ndvi is not None else None,
                    "ndbi_mean": round(ndbi, 3) if ndbi is not None else None,
                    "ndwi_mean": round(ndwi, 3) if ndwi is not None else None,
                    "land_cover_class": lc_class_str
                }
            })
    ...
    validation_stats = {
        ...
        "source": "Multi-Sensor (Landsat 8/9, Sentinel-2, ESA WorldCover v200)",
        "resolution_m": scale,
